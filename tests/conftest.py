...
- DATABASE_URL → 系统临时目录中的进程独立 SQLite（不碰 .env 里的 PG/业务库，
...
- 知识库为空时通过 RAG v2 ingestion 灌入 data/knowledge* 种子，
...
_TEST_DB = Path(tempfile.gettempdir()) / f"buildmate-test-{os.getpid()}.db"
...
async def _seed_knowledge_if_empty() -> None:
    ...
    from scripts.seed_knowledge import seed_markdown_directories

    async with engine.connect() as conn:
        n = (await conn.execute(text("SELECT COUNT(*) FROM knowledge_chunks"))).scalar()
    if n:
        return
    await seed_markdown_directories(
