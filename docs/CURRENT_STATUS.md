...
2026-09-07 Docker 实测：应用镜像构建成功（包含前端生产构建），容器内 `pip check`、OpenCV/OCR/CPU 推理依赖导入、Compose 配置校验通过。独立临时环境中的 PostgreSQL、Redis、RabbitMQ、MinIO/S3、后端及 Worker 已完成以下验证：

- 登录和未登录访问拦截；首页及 `/qa`、`/login` 页面刷新；缺失 API/静态资源仍返回 404。
- 11 份内置知识文档入库，共 187 个片段；问答引用均对应本次实际召回片段。
- S3 文件上传、QA 任务投递及执行成功、重复提交幂等、流式问答正常结束。
- 容器重启后聊天消息及引用恢复；Outbox 领取/重试和审批时间写入通过。
- 现有 Milvus 通过同网络只读连接检查；上述问答使用 `VECTOR_BACKEND=local`。

已修复容器缺少系统依赖与配置/知识文件、PostgreSQL 检查点初始化权限、Worker 未初始化持久记忆、空项目参数类型推断、无时区日期绑定，以及前端历史路由刷新 404。

验证边界：真实模型调用 **Not run**（外发知识片段的调用被自动审批拦截）；Milvus 语义检索、真实 Revit 和可观测栈端到端 **Not run**。本轮 QA 使用离线模型。没有启动或替换现有项目的完整生产 Compose 栈，也没有修改现有数据库/模型数据。Docker 镜像加速地址曾返回 401，本次用官方来源拉取并标记基础镜像后构建成功。
