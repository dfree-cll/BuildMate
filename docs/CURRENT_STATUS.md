...
- 前端投标报告、聊天气泡和 API 错误提示已共用组件级工具；`tests/` 历史 `.tmp_*` 已清空，旧手工冒烟脚本移至 `scripts/manual_e2e/`。
...
2026-09-07 当前增量回归：后端全量测试通过，前端 8 项单元测试通过，前端生产构建通过；包含真实 HTTP/SSE + 独立测试数据库的计划保存/恢复与隔离测试。已在 Edge 中实测 QA 点击发送、中文回车发送、谈判面板发送，以及后端自动刷新后无需重启前端继续发送；六个本地服务刷新后均恢复为 OK。本轮未执行全量 BIM 或真实 Revit 建模；不覆盖下方历史全量基线以外的交付验证。

```text
后端全量回归：820 passed, 2 skipped（2026-09-07）；包含墙体/楼层标高、RAG、任务运行时、Revit 合同、导入兼容及旧入口清理回归。
